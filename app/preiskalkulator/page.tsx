...
function PreiskalkulatorContent() {
  ...
  const tileClass = (selected: boolean) =>
    `relative rounded-2xl p-5 sm:p-6 border-2 transition-all duration-200 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0f] ${
      selected
        ? "bg-[rgba(139,92,246,0.18)] border-[rgba(139,92,246,0.6)] shadow-[0_0_24px_rgba(139,92,246,0.15)]"
        : "bg-white/[0.04] border-white/[0.08] hover:border-[rgba(139,92,246,0.35)] hover:bg-white/[0.06]"
    }`;

  return (
    <main className="ds-app ds-content-section min-h-screen bg-[#0a0a0f] px-4 text-white sm:px-6">
      <div className="mx-auto flex w-full max-w-6xl flex-col items-center">
      {/* Eigenes Fenster: zentriertes Panel ohne Backdrop-Blur (stabil) */}
      <div className="w-full max-w-5xl rounded-3xl border border-white/[0.08] overflow-hidden bg-[#0c0c12] shadow-xl">
        <div className="ds-card-lg p-6 sm:p-8 lg:p-10">
          <div className="flex items-start justify-between gap-4 mb-8">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-[rgba(139,92,246,0.2)] border border-[rgba(139,92,246,0.3)]">
              ...
              <div>
                ...
                <p className="text-white/55 text-sm sm:text-base mt-0.5">
                  Individuelle Kosteneinschätzung in wenigen Schritten
                </p>
              </div>
            </div>
            <Link
              ...
              className="shrink-0 rounded-xl p-2.5 text-white/60 hover:text-white hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] transition-colors"
            >
          ...
          <nav
            ...
            className="flex flex-wrap gap-2 sm:gap-3 mb-10 pb-6 border-b border-white/[0.06]"
          >
            {STEPS.map((s) => (
              <button
                key={s.id}
                ...
                className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${
                  step === s.id
                    ? "bg-[rgba(139,92,246,0.22)] text-white"
                    : "text-white/55 hover:text-white/90 hover:bg-white/[0.06]"
                }`}
              >
                <span
                  className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-xs font-semibold ${
                    step === s.id ? "border-[var(--accent)] bg-[rgba(139,92,246,0.3)]" : "border-white/20"
                  }`}
                >
                  {s.id + 1}
                </span>
          ...
          <div className="lg:col-span-2" ref={stepRef}>
            {step === 0 && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {PROJECT_TYPES.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <span className={selected ? "text-[var(--accent)]" : "text-white/70"}>{icon}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {SCOPES.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <span className={state.scope === opt.value ? "text-[var(--accent)]" : "text-white/70"}>{icon}</span>
                          <span className="text-[15px] font-medium">{opt.label}</span>
                        </button>
                      );
                    })}
                    ...
                    {FEATURE_OPTIONS.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-[15px] font-medium">{opt.label}</span>
                          {price && (
                            <span className={`text-sm font-semibold ${selected ? "text-[var(--accent)]" : "text-white/45"}`}>
            ...
            {step === 5 && quote && (
                <div key="5" className="space-y-8">
                  ...
                  {/* Hero Preis-Karte (High-End, ohne GPU-kritische Effekte) */}
                  <div className="rounded-2xl border border-[rgba(139,92,246,0.35)] bg-[rgba(139,92,246,0.12)] p-6 sm:p-8">
                    <p className="text-white/55 text-sm font-medium uppercase tracking-wider mb-2">Individueller Preisbereich (brutto)</p>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer rounded-xl border border-white/[0.08] bg-white/[0.04] p-4">
                    <input
                      ...
                      className="rounded border-white/30 text-[var(--accent)] focus:ring-[var(--accent)]"
                    />
                  ...
                  {!showOfferForm && !formSuccess && (
                    <button
                      ...
                      className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-base font-medium text-white bg-[var(--accent)] hover:opacity-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0f] transition-opacity shadow-lg shadow-[var(--accent)]/25"
                    >
                  ...
                  {showOfferForm && !formSuccess && (
                    <form onSubmit={handleSubmitOffer} className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.04] p-6">
                      <h3 className="font-medium text-white/95">Kontakt für Angebot</h3>
                      <div>
                        <label htmlFor="lead-name" className="block text-sm text-white/65 mb-1">Name *</label>
                        <input id="lead-name" name="name" required className="w-full rounded-xl bg-white/10 border border-white/20 px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" placeholder="Ihr Name" />
                      </div>
                      <div>
                        <label htmlFor="lead-email" className="block text-sm text-white/65 mb-1">E-Mail *</label>
                        <input id="lead-email" name="email" type="email" required className="w-full rounded-xl bg-white/10 border border-white/20 px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" placeholder="ihre@email.de" />
                      </div>
                      <div>
                        <label htmlFor="lead-company" className="block text-sm text-white/65 mb-1">Unternehmen (optional)</label>
                        <input id="lead-company" name="company" className="w-full rounded-xl bg-white/10 border border-white/20 px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent)]" placeholder="Firma" />
                      </div>
                      <div>
                        <label htmlFor="lead-message" className="block text-sm text-white/65 mb-1">Nachricht (optional)</label>
                        <textarea id="lead-message" name="message" rows={4} defaultValue={prefillMessage} className="w-full rounded-xl bg-white/10 border border-white/20 px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent)] resize-none" placeholder="Ihre Auswahl wird hier vorausgefüllt..." />
                      </div>
                      {formError && <p className="text-sm text-red-400">{formError}</p>}
                      <div className="flex gap-3">
                        <button type="submit" disabled={formSubmitting} className="px-6 py-3 rounded-xl text-base font-medium text-white bg-[var(--accent)] hover:opacity-95 disabled:opacity-60 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
                          {formSubmitting ? "Wird gesendet…" : "Anfrage senden"}
                        </button>
                        <button type="button" onClick={() => { setShowOfferForm(false); setFormError(null); }} className="px-6 py-3 rounded-xl text-base font-medium text-white/80 hover:text-white border border-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
                          Abbrechen
                        </button>
                      </div>
                    </form>
                  ...
                  {formSuccess && (
                    <div className="rounded-2xl border border-[rgba(139,92,246,0.3)] bg-[rgba(139,92,246,0.08)] p-6 text-center">
                      <p className="text-[var(--accent)] font-medium">
                        Vielen Dank. Ihre Anfrage wurde gesendet. Wir melden uns zeitnah.
                      </p>
                    </div>
                  )}
                </div>
            ...
            {step < 5 && (
              <div className="mt-10 flex items-center justify-between gap-4">
                <button
                  ...
                  onClick={prevStep}
                  disabled={step === 0}
                  className="rounded-xl px-4 py-2.5 text-sm font-medium border border-white/20 text-white/70 hover:text-white hover:bg-white/5 disabled:opacity-40 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
                >
                  Zurück
                </button>
                <button
                  ...
                  className="rounded-xl px-6 py-3 text-sm font-medium text-white bg-[var(--accent)] hover:opacity-95 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0f]"
                >
          ...
          <aside className="lg:col-span-1 lg:sticky lg:top-8 lg:self-start rounded-2xl border border-white/[0.08] bg-[rgba(255,255,255,0.03)] p-5 sm:p-6" style={{ minHeight: "200px" }}>
...
export default function PreiskalkulatorPage() {
  return (
    <Suspense
      fallback={
        <main className="ds-app min-h-screen flex items-center justify-center bg-[#0a0a0f] text-white">
          <div className="rounded-2xl border border-white/10 bg-white/5 px-8 py-6 text-white/70">Laden …</div>
        </main>
      }
    >
